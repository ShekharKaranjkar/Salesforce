({
    doInit : function(component, event, helper) {
      // alert( eventrec.Sign__c =returnval.OwnerId);
//       component.set("v.vehiclid",component.get('v.Vehiclerec'));
        
          component.set("v.Issueid",component.get('v.recordId'));
        var action = component.get("c.getissuecheckvalue");
        var  newEvent;
        ...
        action.setCallback(this, function(a) {
            ...
            if (state === "SUCCESS") {
                ...
                component.set("v.issuerec",returnval);
                // component.set("v.Vehiclerec",returnval);
                
                 var vehrec = {'Id':returnval.Vehicle__r.id,'Name':returnval.Vehicle__r.Name};
                component.set("v.Vehiclerec", vehrec);
                
                var carrec = {'Id':returnval.OwnerId,'Name':returnval.Owner.Name};
                component.set("v.userobj", carrec);
                var  vehicle = component.get('v.issuerec');
                var vehiclarray = [];
                vehiclarray.push(vehicle);
                component.set("v.selectedLookUpRecords", vehiclarray);
                var eventrec = component.get("v.driverList");
                eventrec.Issues__c =returnval.Id;
                eventrec.Vehicle__c =returnval.Vehicle__r.id ;
                //eventrec.Sign__c =returnval.OwnerId;
    ...
    OnSubmit:function(component, event, helper){
       ...
       // alert('Save'+component.get("V.issuerec.id")); 
        debugger;      
        var newCheck = component.get("v.driverList");
         // alert('ownerId'+newCheck.ownerId);
        ...
        var action = component.get("c.createIssueAccessriesRecord");
        ...
        action.setCallback(this, function(a) {
            ...
            var state = a.getState();
            if(state == "SUCCESS"){
                console.log('state'+JSON.stringify(component.get('v.issuerec')));
                ...
                dismissQuickAction.fire();
                              var temp=component.get('v.issuerec');
    ...
    handlemultipleComponentEvent : function(component, event, helper) {
        ...
        var IsServiceProvider2 = event.getParam("IsServiceProvider2");
         var IsServiceProvider3 = event.getParam("IsServiceProvider3");
        var eventrec = component.get("v.driverList");
        if(IsServiceProvider == true){
            eventrec.Service_Center__c =selecteduserGetFromEvent.Id;
            component.set("v.driverList", eventrec);
            //alert('Serviec Provider Id:'+selecteduserGetFromEvent.Id);
            ...
            eventrec.Issues__c =selecteduserGetFromEvent.Id;
            component.set("v.driverList", eventrec);
        }
            else {
                alert(selecteduserGetFromEvent.Id);
                eventrec.Sign__c =selecteduserGetFromEvent.Id;//005
                component.set("v.driverList", eventrec);
